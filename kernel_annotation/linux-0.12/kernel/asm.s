...
no_error_code:				# 这里是无出错号处理的入口处，见下面第 56 行等
	...
	push %es
	push %fs
	pushl $0		# "error code"		# "error code" # 将数值 0 作为出错码入栈
	lea 44(%esp),%edx		# 取有效地址，即栈中原调用返回地址处的栈指针位置
	pushl %edx				# 并压入堆栈（即图 8-4 中 esp0 指针入栈） 。
	movl $0x10,%edx			# 初始化段寄存器 ds、 es 和 fs，加载内核数据段选择符
	mov %dx,%ds
	mov %dx,%es
	mov %dx,%fs
# 下行的'*'号表示调用操作数指定地址处的函数，称为间接调用。这句的含义是调用引起本次异常
# 的 C 处理函数，例如 do_divide_error(esp, error_code)等。 在函数返回后， 第 41 行是会将堆栈
# 指针加 8 相当于执行两次 pop 操作，弹出（丢弃）最后入栈的两个 C 函数参数（ 33 行和 35 行入栈
# 的值），让堆栈指针重新指向寄存器 fs 入栈处
	call *%eax
	addl $8,%esp			# 类似： do_divide_error(long esp, long error_code)
	pop %fs
	pop %es
	pop %ds
	popl %ebp
...
error_code:
	xchgl %eax,4(%esp)		# error code <-> %eax	%eax， eax 原来的值被保存在堆栈上。
	xchgl %ebx,(%esp)		# &function <-> %ebx	%ebx， ebx 原来的值被保存在堆栈上。
	pushl %ecx
	pushl %edx
	pushl %edi
	pushl %esi
